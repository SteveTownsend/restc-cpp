...
public:
  ...
  ::restc_cpp::boost_const_buffer ReadSome() override {
    if (auto conn = connection_.lock()) {
      auto timer = IoTimer::Create("IoReaderImpl", cfg_.msReadTimeout, conn);
      ...
      for (size_t retries = 0;; ++retries) {
        ...
        try {
          if (retries != 0u) {
            ...
            RESTC_CPP_LOG_DEBUG_("IoReaderImpl::ReadSome: Waking up. Will try "
                                 "to read from the socket now.");
          }
          ...
          if (ex.code() ==
            ...
            if (retries < cfg_.maxRetries) {
              RESTC_CPP_LOG_DEBUG_("IoReaderImpl::ReadSome: Caught "
          ...
          RESTC_CPP_LOG_DEBUG_(
              "IoReaderImpl::ReadSome: Caught exception: " << ex.what());
          throw;
        }

        RESTC_CPP_LOG_TRACE_("Read #" << bytes << " bytes from " << conn);
    ...
    RESTC_CPP_LOG_DEBUG_(
        "IoReaderImpl::ReadSome: Reached outer scope. Timed out?");
