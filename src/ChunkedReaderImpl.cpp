...
public:
  ...
  static void Log(const ::restc_cpp::boost_const_buffer buffers,
                  const char *tag) {
    const auto buf_len = boost_buffer_size(buffers);

    // At the time of the implementation, there are never multiple buffers.
    RESTC_CPP_LOG_TRACE_(tag
                         << ' ' << "# " << buf_len << " bytes: "
                         << ToPrintable({boost_buffer_cast(buffers), buf_len}));
  }
  ...
  ::restc_cpp::boost_const_buffer ReadSome() override {
    ...
    if (chunk_len_ == 0) {
      ...
      if (chunk_len_ == 0) {
        // Read the trailer
        RESTC_CPP_LOG_TRACE_("ChunkedReaderImpl::ReadSome(): End of chunked "
                             "stream - reading headers");
        stream_->ReadHeaderLines(add_header_);
        stream_->SetEof();
        RESTC_CPP_LOG_TRACE_(
            "ChunkedReaderImpl::ReadSome(): End of chunked stream. Done.");
        return {nullptr, 0};
      }
    }
...
DataReader::ptr_t
DataReader::CreateChunkedReader(add_header_fn_t fn,
                                unique_ptr<DataReaderStream> &&source) {
