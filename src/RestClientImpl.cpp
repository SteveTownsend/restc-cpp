...
public:
  ...
  struct DoneHandlerImpl : public DoneHandler {
    ...
    DoneHandlerImpl(RestClientImpl &parent) : parent_{parent} {
  ...
  public:
    ContextImpl(boost::asio::yield_context &yield, RestClient &rc)
        : yield_{yield}, rc_{rc} {}

    RestClient &GetClient() override { return rc_; }
    boost::asio::yield_context &GetYield() override { return yield_; }

    shared_ptr<Reply> Get(string url) override {
      auto req = Request::Create(url, restc_cpp::Request::Type::GET, rc_);
      return Request(*req);
    }

    shared_ptr<Reply> Post(string url, string body) override {
      auto req =
          Request::Create(url, restc_cpp::Request::Type::POST, rc_,
                          {RequestBody::CreateStringBody(std::move(body))});
      return Request(*req);
    }

    shared_ptr<Reply> Put(string url, string body) override {
      auto req =
          Request::Create(url, restc_cpp::Request::Type::PUT, rc_,
                          {RequestBody::CreateStringBody(std::move(body))});
      return Request(*req);
    }

    shared_ptr<Reply> Delete(string url) override {
      auto req = Request::Create(url, restc_cpp::Request::Type::DELETE, rc_);
      return Request(*req);
    }

    shared_ptr<Reply> Options(string url) override {
      auto req = Request::Create(url, restc_cpp::Request::Type::OPTIONS, rc_);
      return Request(*req);
    }

    shared_ptr<Reply> Head(string url) override {
      auto req = Request::Create(url, restc_cpp::Request::Type::HEAD, rc_);
      return Request(*req);
    }

    shared_ptr<Reply> Patch(string url) override {
      auto req = Request::Create(url, restc_cpp::Request::Type::PATCH, rc_);
      return Request(*req);
    }

    shared_ptr<Reply> Request(restc_cpp::Request &req) override {
  ...
  void Init(const boost::optional<Request::Properties> &properties,
    ...
    if (default_connection_properties_->threads != 1) {
      RESTC_CPP_LOG_WARN_("Init: Compiled without RESTC_CPP_THREADED_CTX: "
                          "Using only one thread!");
      default_connection_properties_->threads = 1;
    }
#endif

    // Make sure we don't re-arrange the vectors while some thread is reading
    // from it
    ...
    work_ = std::make_unique<boost_work>(
        boost::asio::make_work_guard(io_service_->get_executor()));
#else
    work_ = std::make_unique<boost_work>(*io_service_);
#endif

    RESTC_CPP_LOG_TRACE_("Starting " << default_connection_properties_->threads
                                     << " worker thread(s)");
    for (size_t i = 0; i < default_connection_properties_->threads; ++i) {
  ...
  bool IsClosing() const noexcept override { return closed_; }
  ...
  void CloseWhenReady(bool wait) override {
    ...
    if (wait) {
      ...
      // We have to lock/unlock each of them to make sure that all the threads
      // are done
  ...
  void ProcessInWorker(boost::asio::yield_context yield, const prc_fn_t &fn,
  ...
  void Process(const prc_fn_t &fn) override {
    boost::asio::spawn(*io_service_, bind(&RestClientImpl::ProcessInWorker,
                                          this, placeholders::_1, fn, nullptr)
  ...
  future<void> ProcessWithPromise(const prc_fn_t &fn) override {
    ...
    boost::asio::spawn(*io_service_, bind(&RestClientImpl::ProcessInWorker,
                                          this, placeholders::_1, fn, promise)
                                         RESTC_CPP_SPAWN_TRAILER);
...
#ifdef RESTC_CPP_WITH_TLS
  shared_ptr<boost::asio::ssl::context> GetTLSContext() override {
    return tls_context_;
  }
#endif
...
private:
  Request::Properties::ptr_t default_connection_properties_ =
      make_shared<Request::Properties>();
  ...
  void setDefaultSSLContext() {
    tls_context_ = make_shared<boost::asio::ssl::context>(
...
#ifdef RESTC_CPP_WITH_TLS
unique_ptr<RestClient>
RestClient::Create(std::shared_ptr<boost::asio::ssl::context> ctx) {
  boost::optional<Request::Properties> properties;
...
std::unique_ptr<RestClient> RestClient::Create(boost_io_service &ioservice) {
  return make_unique<RestClientImpl>(boost::optional<Request::Properties>{},
                                     ioservice);
}
