...
public:
  ...
  struct KeyHash {
    inline std::size_t operator()(const Key &key) const {
      size_t result(0);
      boost::hash_combine(result, std::hash<int>()(static_cast<int>(key.type)));
      boost::hash_combine(
          result, std::hash<std::string>()(key.endpoint.address().to_string()));
      boost::hash_combine(
          result, std::hash<boost::asio::ip::port_type>()(key.endpoint.port()));
      boost::hash_combine(result,
                          std::hash<int>()(key.endpoint.protocol().family()));
      return result;
    }
  };
  ...
  struct KeyEquals {
    inline bool
    operator()(restc_cpp::ConnectionPoolImpl::Key const &lhs,
               restc_cpp::ConnectionPoolImpl::Key const &rhs) const {
      return lhs.type == rhs.type &&
             lhs.endpoint.address().to_string() ==
                 rhs.endpoint.address().to_string() &&
             lhs.endpoint.port() == rhs.endpoint.port() &&
             lhs.endpoint.protocol().family() ==
                 rhs.endpoint.protocol().family();
    }
  };
  ...
  struct Key {
    ...
    friend bool operator==(restc_cpp::ConnectionPoolImpl::Key &lhs,
                           restc_cpp::ConnectionPoolImpl::Key &rhs);
  ...
  struct Entry {
    ...
    Entry(boost::asio::ip::tcp::endpoint ep,
          const Connection::Type connectionType, Connection::ptr_t conn,
          const Request::Properties &prop)
        : key{std::move(ep), connectionType}, connection{std::move(conn)},
          ttl{prop.cacheTtlSeconds}, created{time(nullptr)} {}
  ...
  public:
    ...
    ConnectionWrapper(Entry::ptr_t entry, release_callback_t on_release)
  ...
  Connection::ptr_t GetConnection(const boost::asio::ip::tcp::endpoint ep,
                                  const Connection::Type connectionType,
                                  bool newConnectionPlease) override {

    if (!newConnectionPlease) {
      if (auto conn = GetFromCache(ep, connectionType)) {
        RESTC_CPP_LOG_TRACE_("Reusing connection from cache " << *conn);
        return conn;
      }
  ...
  auto &GetCtx() const { return owner_.GetIoService(); }
  ...
  void StartTimer() { ScheduleNextCacheCleanup(); }
...
private:
  ...
  void OnCacheCleanup(const boost::system::error_code &error) {
    ...
    const auto now = std::chrono::steady_clock::now();
    {
      ...
      for (auto it = idle_.begin(); !closed_ && it != idle_.end();) {
        ...
        auto expires =
            entry.GetLastUsed() + std::chrono::seconds(entry.GetTtl());
        if (expires < now) {
          RESTC_CPP_LOG_TRACE_("Expiring "
                               << *current->second->GetConnection());
          idle_.erase(current);
        } else {
          RESTC_CPP_LOG_TRACE_(
              "Keeping << "
              << *current->second->GetConnection() << " expieres in "
              << std::chrono::duration_cast<std::chrono::seconds>(expires - now)
                     .count()
              << " seconds ");
        }
      }
  ...
  Connection::ptr_t CreateNew(const boost::asio::ip::tcp::endpoint &ep,
    ...
    auto entry = make_shared<Entry>(
        ep, connectionType, make_shared<ConnectionImpl>(std::move(socket)),
        *properties_);
  ...
  RestClient &owner_;
  unordered_multimap<Key, Entry::ptr_t, KeyHash, KeyEquals> idle_;
  unordered_multimap<Key, std::weak_ptr<Entry>, KeyHash, KeyEquals> in_use_;
...
ConnectionPool::ptr_t ConnectionPool::Create(RestClient &owner) {
