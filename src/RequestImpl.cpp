...
string ref_to_string(boost::string_ref ref) { return {ref.data(), ref.size()}; }

// We support versions of boost prior to the introduction of this convenience
// function
...
boost::asio::ip::address_v6 make_address_v6(const char *str,
  ...
  if (boost::asio::detail::socket_ops::inet_pton(
          BOOST_ASIO_OS_DEF(AF_INET6), str, bytes.data(), &scope_id, ec) <= 0) {
    return {};
  }
...
boost::asio::ip::address_v4 make_address_v4(const char *str,
  ...
  if (boost::asio::detail::socket_ops::inet_pton(
          BOOST_ASIO_OS_DEF(AF_INET), str, &bytes, nullptr, ec) <= 0) {
    return {};
  }
...
const std::string &Request::Proxy::GetName() const {
  static const array<string, 3> names = {"NONE", "HTTP", "SOCKS5"};
...
pair<string, uint16_t> ParseAddress(const std::string &addr) {
  ...
  const uint16_t port_num = static_cast<uint16_t>(stoul(port));
  if (port_num == 0 || port_num > numeric_limits<uint16_t>::max()) {
    throw ParseException{"Port `:<port>` must be a valid IP port number in "s +
                         addr};
  }
...
size_t ValidateCompleteSocks5ConnectReply(const uint8_t *buf, size_t len) {
  if (len < 5) {
    throw RestcCppException{
        "SOCKS5 server connect reply must start at minimum 5 bytes"s};
  }

  if (buf[0] != SOCKS5_VERSION) {
    throw ProtocolException{
        "Wrong/unsupported SOCKS5 version in connect reply: "s +
        to_string(buf[0])};
  }
  ...
  if (len > hdr_len) {
    throw NotSupportedException{
        "SOCKS5: Received more data then the connect response."};
  }

  return hdr_len;
}

void DoSocks5Handshake(Connection &connection, const Url &url,
                       const Request::Properties &properties, Context &ctx) {
  ...
  }
  {
    ...
    if (reply[0] != SOCKS5_VERSION) {
      throw ProtocolException{"Wrong/unsupported SOCKS5 version: "s +
                              to_string(reply[0])};
    }

    if (reply[1] != 0) {
      throw AccessDeniedException{"SOCKS5 Access denied: "s +
                                  to_string(reply[1])};
    }
  }
  ...
  // Send connect parameters
  {
    ...
    RESTC_CPP_LOG_TRACE_("DoSocks5Handshake - saying connect to "
                         << url.GetHost().to_string() << ":" << url.GetPort());
...

  {
    ...
    RESTC_CPP_LOG_TRACE_(
        "DoSocks5Handshake - waiting for connect confirmation - first segment");
    ...
    if (hdr_len > read) {
      ...
      if (hdr_len > reply.size()) {
        throw ProtocolException{
            "SOCKS5 Connect header from the server is too large"};
      }
      RESTC_CPP_LOG_TRACE_("DoSocks5Handshake - waiting for connect "
                           "confirmation - second segment");
...
public:
  struct RedirectException {
    ...
    RedirectException(const RedirectException &rhs)
        : code(rhs.code), url(rhs.url), redirectReply(rhs.redirectReply) {}
    RedirectException(RedirectException &&rhs)
        : code(std::move(rhs.code)), url(std::move(rhs.url)),
          redirectReply(rhs.redirectReply) {}

    RedirectException(int redirCode, string redirUrl,
                      std::shared_ptr<Reply> reply)
        : code{redirCode}, url{std::move(redirUrl)}, redirectReply(reply) {}
    ...
    [[nodiscard]] std::shared_ptr<Reply> TakeRedirectReply() const {
  ...
  RequestImpl(std::string url, const Type requestType, RestClient &owner,
      ...
      : url_{std::move(url)}, parsed_url_{url_.c_str()},
        request_type_{requestType}, body_{std::move(body)}, owner_{owner} {
    if (args || headers || auth) {
      ...
      if (args) {
        properties_->args.insert(properties_->args.end(), args->begin(),
                                 args->end());
      }
  ...
  static std::string Base64Encode(const std::string &in) {
    ...
    if (valb > -magic_6) {
      out.push_back(
          alphabeth[((val << magic_8) >> (valb + magic_8)) & magic_3f]);
    }
  ...
  void SetAuth(const Auth &auth) {
    ...
    properties_->headers[authorization] = basic_sp + Base64Encode(pre_base);
  ...
  shared_ptr<Reply> Execute(Context &ctx) override {
    int redirects = 0;
    while (true) {
      try {
        ...
        // if this is not an absolute URL treat it as relative to request per
        // https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Location#directives
        boost::core::string_view target = url;
        boost::system::result<boost::urls::url_view> parsed_uri =
            boost::urls::parse_uri(target);
        if (parsed_uri.has_error()) {
          boost::core::string_view root = url_;
          boost::system::result<boost::urls::url> mutable_root =
              boost::urls::parse_uri(root).value();
          mutable_root->set_path(target);
          url = mutable_root->c_str();
        }

        // callback may stop the follow-chaining
        if (properties_->redirectFn &&
            !properties_->redirectFn(ex.GetCode(), url,
                                     ex.GetRedirectReply())) {
          return ex.TakeRedirectReply();
        }
        ...
        RESTC_CPP_LOG_DEBUG_("Redirecting (" << ex.GetCode() << ") '" << url_
                                             << "' --> '" << url << "') ");
        url_ = std::move(url);
...
private:
  ...
  std::string BuildOutgoingRequest() {
    ...
    if (add_url_args_) {
      ...
      for (const auto &arg : properties_->args) {
        ...
        request_buffer << url_encode(arg.name) << '=' << url_encode(arg.value);
      }
    } else {
  ...
  std::pair<std::string, std::string> GetRequestEndpoint() {
    ...
    if (proxy_type == Request::Proxy::Type::HTTP) {
      ...
      RESTC_CPP_LOG_TRACE_("Using " << properties_->proxy.GetName()
                                    << " Proxy at: " << proxy.GetHost() << ':'
                                    << proxy.GetPort());

      return {proxy.GetHost().to_string(), proxy.GetPort().to_string()};
    }
  ...
  ToEp(const std::string &endp, const protocolT &protocol, Context &ctx) const {
    string host;
    string port;
    ...
    for (auto it = results.begin(); it != results.end(); ++it) {
      ...
      RESTC_CPP_LOG_TRACE_(
          "ep=" << endpoint << ", protocol=" << endpoint.protocol().protocol());
    ...
    RESTC_CPP_LOG_ERROR_("Failed to resolve endpoint "
                         << endp << " with protocol " << protocol.protocol());
    throw FailedToResolveEndpointException{"Failed to resolve endpoint: "s +
                                           endp};
  }
      ...
      for (size_t retries = 0; retries < properties_->maxRetries; ++retries) {
        ...
        if (!properties_->bindToLocalAddress.empty()) {
          ...
          if (!prot_filter.empty()) {
            if (std::find(prot_filter.begin(), prot_filter.end(),
                          endpoint.protocol()) == prot_filter.end()) {
              RESTC_CPP_LOG_TRACE_(
                  "Filtered out (protocol mismatch) local address: "
          ...
          RESTC_CPP_LOG_TRACE_(
              "Binding to local address: " << properties_->bindToLocalAddress);

          boost::system::error_code ec;
          auto local_ep =
              ToEp(properties_->bindToLocalAddress, endpoint.protocol(), ctx);
        ...
        auto timer = IoTimer::Create(timer_name, properties_->connectTimeoutMs,
                                     connection);
        ...
        try {
          if (retries != 0u) {
            ...
            RESTC_CPP_LOG_DEBUG_("RequestImpl::Connect: Waking up. Will try to "
                                 "read from the socket now.");
          }
          ...
          if (properties_->proxy.type == Proxy::Type::SOCKS5) {
            connection->GetSocket().SetAfterConnectCallback([&]() {
              ...
              RESTC_CPP_LOG_TRACE_(
                  "RequestImpl::Connect: Leaving Socks5 callback");
            });
          }

          RESTC_CPP_LOG_TRACE_("RequestImpl::Connect: calling AsyncConnect --> "
                               << endpoint);

          // 2025-01-18: [jgaa] Changing the hostname from the one digged up by
          // the resolver in older versions of boost to the one we actually want
          // to connect to. This affects certificate validation for TLS. I
          // believe this is the correct way to do it.
          connection->GetSocket().AsyncConnect(
              endpoint, host, properties_->tcpNodelay, ctx.GetYield());
          RESTC_CPP_LOG_TRACE_("RequestImpl::Connect: OK AsyncConnect --> "
                               << endpoint);
          return connection;
        } catch (const boost::system::system_error &ex) {
          RESTC_CPP_LOG_TRACE_("RequestImpl::Connect:: Caught "
                               "boost::system::system_error exception: \""
                               << ex.what() << "\". Will close connection "
                               << *connection);
          ...
          RESTC_CPP_LOG_WARN_(
              "Connect to " << endpoint << " failed with exception type: "
  ...
  void SendRequestPayload(Context & /*ctx*/, write_buffers_t write_buffer) {
    ...
    while (boost::asio::buffer_size(write_buffer) != 0u) {
      auto timer =
          IoTimer::Create(timer_name, properties_->sendTimeoutMs, connection_);
  ...
  DataWriter &SendRequest(Context &ctx) override {
    ...
    if (body_) {
      if (body_->GetType() == RequestBody::Type::FIXED_SIZE) {
        writer_ = DataWriter::CreatePlainWriter(body_->GetFixedSize(),
                                                std::move(writer_));
      } else {
    ...
    RESTC_CPP_LOG_DEBUG_("Sent " << Verb(request_type_) << " request to '"
                                 << url_ << "' " << *connection_);
  ...
  shared_ptr<Reply> GetReply(Context &ctx) override {
    // per https://developer.mozilla.org/en-US/docs/Web/HTTP/Redirections
    constexpr auto http_301 = 301;
    constexpr auto http_302 = 302;
    constexpr auto http_303 = 303;
    constexpr auto http_307 = 307;
    constexpr auto http_308 = 308;
    ...
    try {
      ...
      RESTC_CPP_LOG_DEBUG_(
          "GetReply: exception from StartReceiveFromServer: " << ex.what());
      throw;
    }

    RESTC_CPP_LOG_TRACE_("GetReply: Returned from StartReceiveFromServer. code="
                         << reply->GetResponseCode());

    const auto http_code = reply->GetResponseCode();
    if (http_code == http_301 || http_code == http_302 ||
        http_code == http_303 || http_code == http_307 ||
        http_code == http_308) {
      ...
      RESTC_CPP_LOG_TRACE_(
          "GetReply: RedirectException. location=" << *redirect_location);
  ...
  shared_ptr<Reply> DoExecute(Context &ctx) {
    SendRequest(ctx);
    return GetReply(ctx);
  }
...
Request::Create(const std::string &url, const Type requestType,
                RestClient &owner, std::unique_ptr<RequestBody> body,
                const boost::optional<args_t> &args,
                const boost::optional<headers_t> &headers,
                const boost::optional<auth_t> &auth) {

  return make_unique<RequestImpl>(url, requestType, owner, std::move(body),
                                  args, headers, auth);
}
