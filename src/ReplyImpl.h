...
public:
  ...
  ReplyImpl(Connection::ptr_t connection, Context &ctx, RestClient &owner,
  ...
  int GetResponseCode() const override { return response_.status_code; }

  const HttpResponse &GetHttpResponse() const override { return response_; }

  boost_const_buffer GetSomeData() override;

  string GetBodyAsString(size_t maxSize = RESTC_CPP_SANE_DATA_LIMIT) override;

  void fetchAndIgnore() override;

  bool MoreDataToRead() override { return !IsEof(); }

  boost::uuids::uuid GetConnectionId() const override { return connection_id_; }

  static std::shared_ptr<ReplyImpl>
  Create(Connection::ptr_t connection, Context &ctx, RestClient &owner,
         Request::Properties::ptr_t &properties, Request::Type type);

  static boost::string_ref b2sr(boost_const_buffer buffer) {
    return {boost_buffer_cast(buffer), boost::asio::buffer_size(buffer)};
  }

  bool IsEof() const { return !reader_ || reader_->IsEof(); }
