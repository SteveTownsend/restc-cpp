...
ReplyImpl::ReplyImpl(Connection::ptr_t connection, Context &ctx,
                     ...
                     Request::Type type)
    : connection_{std::move(connection)}, ctx_{ctx}, properties_{properties},
      owner_{owner},
...
ReplyImpl::ReplyImpl(Connection::ptr_t connection, Context &ctx,
    ...
    : connection_{std::move(connection)}, ctx_{ctx},
      properties_{owner.GetConnectionProperties()}, owner_{owner},
      connection_id_(connection_ ? connection_->GetId()
                                 : boost::uuids::random_generator()()),
      request_type_{type} {}
...
void ReplyImpl::StartReceiveFromServer(DataReader::ptr_t &&reader) {
  ...
  auto timer =
      IoTimer::Create(timer_name, properties_->replyTimeoutMs, connection_);
...
void ReplyImpl::HandleContentType(unique_ptr<DataReaderStream> &&stream) {
  ...
  if (request_type_ == Request::Type::HEAD) {
    ...
    reader_ =
        DataReader::CreatePlainReader(*content_length_, std::move(stream));
  } else {
    auto te = GetHeader(transfer_encoding_name);
    if (te && ciEqLibC()(*te, chunked_name)) {
      reader_ = DataReader::CreateChunkedReader(
...
void ReplyImpl::HandleDecompression() {
  ...
  for (auto it = tok.begin(); it != tok.end(); ++it) {
    ...
    {
      RESTC_CPP_LOG_ERROR_("Unsupported compression: '" << url_encode(*it)
                                                        << "' from server on "
                                                        << *connection_);
...
::restc_cpp::boost_const_buffer ReplyImpl::GetSomeData() {
  auto rval = reader_ ? reader_->ReadSome()
...
string ReplyImpl::GetBodyAsString(const size_t maxSize) {
  ...
  while (!IsEof()) {
    ...
    if ((buffer.size() + buffer_size) >= maxSize) {
      throw ConstraintException("Too much data for the curent buffer limit.");
    }

    buffer.append(boost_buffer_cast(data), buffer_size);
  }
...
void ReplyImpl::ReleaseConnection() {
  if (connection_ && do_close_connection_) {
    RESTC_CPP_LOG_TRACE_(
        "Closing connection because do_close_connection_ is true: "
        << *connection_);
...
std::shared_ptr<ReplyImpl>
ReplyImpl::Create(Connection::ptr_t connection, Context &ctx, RestClient &owner,
                  Request::Properties::ptr_t &properties, Request::Type type) {

  return make_shared<ReplyImpl>(std::move(connection), ctx, owner, properties,
                                type);
}
