...
public:
  ...
  enum class Type { GET, POST, PUT, DELETE, OPTIONS, HEAD, PATCH };

  class Properties {
  public:
    using ptr_t = std::shared_ptr<Properties>;
    using redirect_fn_t =
        std::function<bool(int code, std::string &url, const Reply &reply)>;
    ...
    bool throwOnHttpError =
        true; // If false, the user must detect and deal with the error
  };
   ...
   */
  virtual std::shared_ptr<Reply> GetReply(Context &ctx) = 0;
   ...
   * in the properties.
   */
  virtual std::shared_ptr<Reply> Execute(Context &ctx) = 0;

  virtual ~Request() = default;

  static std::unique_ptr<Request>
  Create(const std::string &url, const Type requestType, RestClient &owner,
...
public:
   ...
   */
  virtual std::string
  GetBodyAsString(size_t maxSize = RESTC_CPP_SANE_DATA_LIMIT) = 0;
...
public:
  ...
  virtual std::shared_ptr<Reply> Get(std::string url) = 0;

  /*! Send a POST request asynchronously to the server. */
  virtual std::shared_ptr<Reply> Post(std::string url, std::string body) = 0;

  /*! Send a PUT request asynchronously to the server. */
  virtual std::shared_ptr<Reply> Put(std::string url, std::string body) = 0;

  /*! Send a DELETE request asynchronously to the server. */
  virtual std::shared_ptr<Reply> Delete(std::string url) = 0;

  /*! Send a OPTIONS request asynchronously to the server. */
  virtual std::shared_ptr<Reply> Options(std::string url) = 0;

  /*! Send a HEAD request asynchronously to the server. */
  virtual std::shared_ptr<Reply> Head(std::string url) = 0;

  /*! Send a PATCH request asynchronously to the server. */
  virtual std::shared_ptr<Reply> Patch(std::string url) = 0;
   ...
   */
  virtual std::shared_ptr<Reply> Request(Request &req) = 0;
  ...
  /*! Asynchronously sleep for a period */
  virtual void Sleep(const uint64_t ms) = 0;
...
public:
  ...
  template <typename T>
  T ProcessWithYield(const std::function<T(Context &ctx)> &fn,
  ...
  Create(std::shared_ptr<::boost::asio::ssl::context> ctx);
  static std::unique_ptr<RestClient>
  Create(std::shared_ptr<::boost::asio::ssl::context> ctx,
  ...
  static std::unique_ptr<RestClient> Create(boost_io_service &ioservice);
