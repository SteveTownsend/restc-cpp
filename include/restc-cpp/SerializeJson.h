...
struct JsonFieldMapping {
  ...
  const std::string &to_json_name(const std::string &name) const noexcept {
  ...
  const std::string &to_native_name(const std::string &name) const noexcept {
    for (const auto &entry : entries) {
...
public:
  ...
  bool HaveParent() const noexcept { return parent_ != nullptr; }
...
struct SerializeProperties {
  SerializeProperties()
      : max_memory_consumption{GetDefaultMaxMemoryConsumption()},
        ignore_empty_fileds{true}, ignore_unknown_properties{true} {}
  ...
  constexpr static uint64_t GetDefaultMaxMemoryConsumption() {
    return 1024 * 1024;
  }
...
template <typename T> struct type_conv {
  ...
  using noconst_ref_type_t =
      typename std::add_lvalue_reference<native_field_type_t>::type;
};

template <typename T, typename fnT> struct on_name_and_value {
  on_name_and_value(fnT fn) : fn_{fn} {}
...
template <typename T> struct get_len {
  size_t operator()(const T &v) const { return sizeof(v); }
};

template <> struct get_len<const std::string &> {
...
template <> struct get_len<std::string &> {
  size_t operator()(const std::string &v) const {
    assert(false); // oops!
...
#if !defined(_MSC_VER) ||                                                      \
    (_MSC_VER >= 1910) // g++, clang, Visual Studio from 2017

template <
    typename varT, typename valT,
    typename std::enable_if<
        ((std::is_integral<varT>::value && std::is_integral<valT>::value) ||
         (std::is_floating_point<varT>::value &&
          std::is_floating_point<valT>::value)) &&
        !std::is_assignable<varT &, const valT &>::value>::type * = nullptr>
...
template <
    ...
    typename std::enable_if<
        ...
        !((std::is_integral<varT>::value && std::is_integral<valT>::value) ||
          (std::is_floating_point<varT>::value &&
           std::is_floating_point<valT>::value))>::type * = nullptr>
void assign_value(varT &var, const valT &val) {
  assert(false);
...
// The C++ compiler from hell cannot select the correct template from the
// generics above.

template <typename varT, typename valT> void assign_value(varT var, valT val) {
  RESTC_CPP_LOG_ERROR_("assign_value: Invalid data conversion from "
                       << RESTC_CPP_TYPENAME(varT) << " to "
                       << RESTC_CPP_TYPENAME(valT));
  assert(false);
  throw ParseException("assign_value: Invalid data conversion from ");
}

template <> void assign_value(int &var, const int &val) { var = val; }

template <> void assign_value(unsigned int &var, const int &val) {
  var = static_cast<unsigned int>(val);
}

template <> void assign_value(unsigned int &var, const unsigned int &val) {
  var = val;
}

template <> void assign_value(int &var, const unsigned int &val) {
  var = static_cast<int>(val);
}

template <> void assign_value(int &var, const bool &val) {
  var = static_cast<int>(val);
}

template <> void assign_value(unsigned int &var, const bool &val) {
  var = static_cast<int>(val);
}

template <> void assign_value(std::int64_t &var, const int &val) { var = val; }

template <> void assign_value(std::int64_t &var, const unsigned int &val) {
  var = static_cast<std::int64_t>(val);
}

template <> void assign_value(std::int64_t &var, const std::int64_t &val) {
  var = val;
}

template <> void assign_value(std::int64_t &var, const std::uint64_t &val) {
  var = static_cast<std::int64_t>(val);
}

template <> void assign_value(std::uint64_t &var, const int &val) {
  var = static_cast<std::uint64_t>(val);
}

template <> void assign_value(std::uint64_t &var, const unsigned int &val) {
  var = val;
}

template <> void assign_value(std::uint64_t &var, const std::int64_t &val) {
  var = static_cast<std::uint64_t>(val);
}

template <> void assign_value(std::uint64_t &var, const std::uint64_t &val) {
  var = val;
}

template <> void assign_value(bool &var, const int &val) {
  var = val ? true : false;
}

template <> void assign_value(bool &var, const unsigned int &val) {
  var = val ? true : false;
}

template <> void assign_value(double &var, const double &val) { var = val; }

template <> void assign_value(float &var, const double &val) {
  var = static_cast<float>(val);
}

template <> void assign_value(short &var, const int &val) {
  var = static_cast<short>(val);
}

template <> void assign_value(short &var, const unsigned int &val) {
  var = static_cast<short>(val);
}

template <> void assign_value(unsigned short &var, const int &val) {
  var = static_cast<unsigned short>(val);
}

template <> void assign_value(unsigned short &var, const unsigned int &val) {
  var = static_cast<unsigned short>(val);
}

template <> void assign_value(char &var, const int &val) {
  var = static_cast<char>(val);
}

template <> void assign_value(char &var, const unsigned int &val) {
  var = static_cast<char>(val);
}

template <> void assign_value(unsigned char &var, const int &val) {
  var = static_cast<unsigned char>(val);
}

template <> void assign_value(unsigned char &var, const unsigned int &val) {
  var = static_cast<unsigned char>(val);
}

template <> void assign_value(std::string &var, const std::string &val) {
  var = val;
}
...
public:
  ...
  bool Bool(bool b) override { return true; }
  ...
  bool String(const char *, std::size_t Tlength, bool) override { return true; }

  bool RawNumber(const char *, std::size_t, bool) override { return true; }
...
#ifdef RESTC_CPP_LOG_JSON_SERIALIZATION
    RESTC_CPP_LOG_TRACE_(
        "   Skipping json key: " << boost::string_ref(str, length));
#endif
    return true;
  }
...
public:
  ...
  RapidJsonDeserializer(data_t &object)
      : RapidJsonDeserializerBase(nullptr), object_{object},
  ...
  explicit RapidJsonDeserializer(data_t &object,
      ...
      : RapidJsonDeserializerBase(parent), object_{object},
        properties_{properties}, bytes_{maxBytes} {
  ...
  bool String(const char *str, std::size_t length, bool copy) override {
    assert(((state_ == State::RECURSED) && recursed_to_) || !recursed_to_);
    return recursed_to_ ? recursed_to_->String(str, length, copy)
  ...
  bool Key(const char *str, std::size_t length, bool copy) override {
    assert(((state_ == State::RECURSED) && recursed_to_) || !recursed_to_);
    return recursed_to_ ? recursed_to_->Key(str, length, copy)
                        : DoKey(str, length, copy);
  }

  bool EndObject(std::size_t memberCount) override {
    assert(((state_ == State::RECURSED) && recursed_to_) || !recursed_to_);
    return recursed_to_ ? recursed_to_->EndObject(memberCount)
  ...
  bool EndArray(std::size_t elementCount) override {
    assert(((state_ == State::RECURSED) && recursed_to_) || !recursed_to_);
    return recursed_to_ ? recursed_to_->EndArray(elementCount)
                        : DoEndArray(elementCount);
  }

private:
  template <typename classT, typename itemT>
  void DoRecurseToMember(
      itemT &item,
      typename std::enable_if<
          boost::fusion::traits::is_sequence<classT>::value ||
          is_container<classT>::value || is_map<classT>::value>::type * = 0) {
  ...
  void RecurseToContainerValue(
      typename std::enable_if<
          (boost::fusion::traits::is_sequence<
               typename dataT::value_type>::value ||
           is_container<typename dataT::value_type>::value) &&
          is_container<dataT>::value>::type * = 0) {
  ...
  template <typename dataT>
  void RecurseToContainerValue(
      typename std::enable_if<!is_container<dataT>::value>::type * = 0) {
    assert(false);
  }
  ...
  template <typename dataT>
  void
  RecurseToMember(typename std::enable_if<is_map<dataT>::value>::type * = 0) {

    using native_type_t =
        typename std::remove_const<typename std::remove_reference<
            typename dataT::mapped_type>::type>::type;
    recursed_to_ = std::make_unique<RapidJsonDeserializer<native_type_t>>(
  ...
  bool SetValueOnMember(
      const argT &new_value,
      typename std::enable_if<
          boost::fusion::traits::is_sequence<dataT>::value>::type * = 0) {
    assert(!current_name_.empty());
  ...
  template <typename dataT, typename argT>
  bool
  SetValueOnMember(const argT &val,
                   typename std::enable_if<is_map<dataT>::value>::type * = 0) {
    assert(!current_name_.empty());

    AddBytes(
        get_len<argT>()(val) + sizeof(std::string) + current_name_.size() +
  ...
  template <typename argT> bool SetValue(argT val) {
#ifdef RESTC_CPP_LOG_JSON_SERIALIZATION
    ...
    RESTC_CPP_LOG_ERROR_(RESTC_CPP_TYPENAME(data_t)
                         << " SetValue: " << current_name_
                         << " State: " << to_string(state_) << " Value type"
                         << RESTC_CPP_TYPENAME(argT));
  ...
  bool DoBool(bool b) { return SetValue(b); }

  bool DoInt(int i) { return SetValue(i); }

  bool DoUint(unsigned u) { return SetValue(u); }

  bool DoInt64(int64_t i) { return SetValue(i); }

  bool DoUint64(uint64_t u) { return SetValue(u); }

  bool DoDouble(double d) { return SetValue(d); }
...
#ifdef RESTC_CPP_LOG_JSON_SERIALIZATION
    RESTC_CPP_LOG_TRACE_(RESTC_CPP_TYPENAME(data_t) << " OnChildIsDone");
#endif
...
template <typename T>
constexpr bool is_empty_field_(
    const T &value,
    typename std::enable_if<std::is_integral<T>::value ||
...
template <typename T> constexpr bool is_empty_field(T &&value) {
  using data_type =
      typename std::remove_const<typename std::remove_reference<T>::type>::type;
...
void do_serialize(
    const dataT &object, serializerT &serializer,
    const serialize_properties_t &properties,
    typename std::enable_if<!boost::fusion::traits::is_sequence<dataT>::value &&
...
void do_serialize(
    ...
    typename std::enable_if<std::is_integral<dataT>::value ||
                            std::is_floating_point<dataT>::value>::type * = 0) {
...
void do_serialize(
    const dataT &object, serializerT &serializer,
    const serialize_properties_t &properties,
    typename std::enable_if<std::is_same<dataT, std::string>::value>::type * =
        0) {

  serializer.String(object.c_str(),
                    static_cast<rapidjson::SizeType>(object.size()), true);
};
...
void do_serialize(
    ...
    typename std::enable_if<is_container<dataT>::value>::type * = 0) {
#ifdef RESTC_CPP_LOG_JSON_SERIALIZATION
  RESTC_CPP_LOG_TRACE_(RESTC_CPP_TYPENAME(dataT) << " StartArray: ");
#endif
...
#ifdef RESTC_CPP_LOG_JSON_SERIALIZATION
  RESTC_CPP_LOG_TRACE_(RESTC_CPP_TYPENAME(dataT) << " EndArray: ");
#endif
  serializer.EndArray();
};
...
#ifdef RESTC_CPP_LOG_JSON_SERIALIZATION
  RESTC_CPP_LOG_TRACE_(RESTC_CPP_TYPENAME(dataT) << " StartMap: ");
#endif
  serializer.StartObject();

  for (const auto &v : object) {

    using native_field_type_t =
        typename std::remove_const<typename std::remove_reference<
            typename dataT::mapped_type>::type>::type;
...
#ifdef RESTC_CPP_LOG_JSON_SERIALIZATION
  RESTC_CPP_LOG_TRACE_(RESTC_CPP_TYPENAME(dataT) << " EndMap: ");
#endif
...
void do_serialize(
    ...
    const serialize_properties_t &properties,
    typename std::enable_if<
        boost::fusion::traits::is_sequence<dataT>::value>::type *) {

  serializer.StartObject();
#ifdef RESTC_CPP_LOG_JSON_SERIALIZATION
  RESTC_CPP_LOG_TRACE_(RESTC_CPP_TYPENAME(dataT) << " StartObject: ");
#endif
  auto fn = [&](const char *name, auto &val) {
#ifdef RESTC_CPP_LOG_JSON_SERIALIZATION
    RESTC_CPP_LOG_TRACE_(RESTC_CPP_TYPENAME(dataT) << " Key: " << name);
#endif
    ...
    if (properties.excluded_names && properties.is_excluded(name)) {
#ifdef RESTC_CPP_LOG_JSON_SERIALIZATION
      RESTC_CPP_LOG_TRACE_(RESTC_CPP_TYPENAME(dataT)
...
#ifdef RESTC_CPP_LOG_JSON_SERIALIZATION
  RESTC_CPP_LOG_TRACE_(RESTC_CPP_TYPENAME(dataT) << " EndObject: ");
#endif
  serializer.EndObject();
};
} // namespace
...
template <typename objectT, typename serializerT> class RapidJsonSerializer {
public:
  using data_t = typename std::remove_const<
  ...
  // See
  // https://github.com/miloyip/rapidjson/blob/master/doc/sax.md#writer-writer
  void Serialize() { do_serialize<data_t>(object_, serializer_, properties_); }
 ...
 *
 */
template <typename T> class RapidJsonInserter {
public:
...
template <typename dataT>
void SerializeFromJson(dataT &rootData, std::istream &stream,
                       const serialize_properties_t &properties) {
...
/*! Serialize a reply to a C++ class instance */
template <typename dataT>
void SerializeFromJson(dataT &rootData, std::shared_ptr<Reply> &&reply) {
  serialize_properties_t properties;
  SerializeFromJson(rootData, *reply, properties);
}
...
void SerializeFromJson(
    dataT &rootData, Reply &reply, const JsonFieldMapping *nameMapper,
...
template <typename dataT>
void SerializeToJson(dataT &rootData, std::ostream &ostream,
                     const serialize_properties_t &properties = {}) {
