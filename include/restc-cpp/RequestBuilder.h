...
public:
  ...
  RequestBuilder(Context &ctx) : ctx_{&ctx} {}
   ...
   * duplicate headers for headers that needs to be unique.
   */
  RequestBuilder &AddHeaders(boost::optional<Request::headers_t> headers) {
    if (headers) {
   ...
   */
  RequestBuilder &Argument(std::string name, std::string value) {
    if (!args_) {
   ...
   *
   */
  template <typename fnT> RequestBuilder &DataProvider(const fnT &fn) {
    assert(!body_);
   ...
   */
  template <typename T>
  RequestBuilder &Data(const T &data, const JsonFieldMapping *mapping = nullptr,
  ...
  std::unique_ptr<Request> Build() {
...
#endif
    auto req = Request::Create(url_, type_, ctx_->GetClient(), std::move(body_),
                               args_, headers_, auth_);
    ...
    if (properties_) {
      ...
      properties_->args.insert(properties_->args.end(), orig.args.begin(),
                               orig.args.end());
