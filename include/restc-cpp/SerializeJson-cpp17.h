...
template <typename T> size_t get_len(const T &v) {
...
template <typename T>
constexpr auto has_std_to_string_implementation(int)
    -> decltype(std::is_same_v<std::string,
...
template <typename T> bool is_digits_only(const T &vect, bool signedFlag) {
  size_t cnt = 0;
  for (auto v : vect) {
...
void assign_value(varT &var, const valT &val) {
  if constexpr (std::is_same_v<valT, std::nullptr_t>) {
    ...
    RESTC_CPP_LOG_TRACE_(
        "assign_value: "
        << "Don't know how to assign null-value/empty value to type "
        << RESTC_CPP_TYPENAME(varT));
    ...
    if constexpr (std::is_same_v<std::string, valT>) {
      if (val == "true" || val == "yes" || atoi(val.c_str()) > 0) {
        var = true;
      } else if (val.empty() || val == "false" || val == "no" ||
                 (val.at(0) == '0' && atoi(val.c_str()) == 0)) {
        var = false;
      } else {
        throw ParseException(
            "assign_value: Invalid data conversion from string to bool");
      }
    ...
    if (is_digits_only(val, true)) {
      ...
      throw ParseException(
          "assign_value: Invalid data conversion from string to int*_t");
    }
    ...
    if (is_digits_only(val, false)) {
      ...
      throw ParseException(
          "assign_value: Invalid data conversion from string to uint*_t");
    }
  } else {
...
private:
  ...
  void DoRecurseToMember(itemT &item) {
    using native_field_type_t = typename std::remove_const<
        typename std::remove_reference<itemT>::type>::type;
    if constexpr (is_optional<native_field_type_t>::value) {
      using val_t = typename std::remove_const<
          typename native_field_type_t::value_type>::type;
      const_cast<native_field_type_t &>(item) = val_t{};
      // DoRecurseToMember<native_field_type_t>(item.value());
      DoRecurseToMember<typename std::remove_const<
          typename std::remove_reference<decltype(item.value())>::type>::type>(
          item.value());
    } else if constexpr (boost::fusion::traits::is_sequence<typeT>::value ||
                         is_container<typeT>::value || is_map<typeT>::value) {
      // using const_field_type_t = decltype(item);
      // using native_field_type_t = typename std::remove_const<typename
      // std::remove_reference<const_field_type_t>::type>::type;
      using field_type_t =
          typename std::add_lvalue_reference<native_field_type_t>::type;
  ...
  template <typename dataT> void RecurseToContainerValue() {
    if constexpr (is_container<dataT>::value) {
      if constexpr (boost::fusion::traits::is_sequence<
        ...
        using native_type_t =
            typename std::remove_const<typename std::remove_reference<
                typename dataT::value_type>::type>::type;
    ...
    throw ParseException(
        std::string{"RecurseToContainerValue: Unexpected type: "} +
        RESTC_CPP_TYPENAME(dataT));
  }

  template <typename dataT> void RecurseToMember() {
    assert(!recursed_to_);
  ...
  bool SetValueOnMember(const argT &val) {
    ...
    if constexpr (boost::fusion::traits::is_sequence<dataT>::value) {
      ...
      if (!found) {
        assert(!found);
        RESTC_CPP_LOG_TRACE_(
            "SetValueOnMember(): Failed to find property-name '"
            << current_name_ << "' in C++ class '" << RESTC_CPP_TYPENAME(dataT)
            << "' when serializing from JSON.");
    ...
    if constexpr (is_map<dataT>::value) {
      assert(!current_name_.empty());
      AddBytes(
          get_len(val) + sizeof(std::string) + current_name_.size() +
  ...
  void SetValueInArray(const argT &val) {
    if constexpr (is_container<dataT>::value) {
      if constexpr (!boost::fusion::traits::is_sequence<
        ...
        object_.push_back({});
        assign_value<decltype(object_.back()), decltype(val)>(object_.back(),
                                                              val);
      } else {
  ...
  bool DoNull() { return SetValue(nullptr); }
...
template <typename T> constexpr bool is_empty_field_(const T &value) {
  ...
  if constexpr //(has_empty_method<T>()) {
      (std::is_same_v<std::string, T> || is_map<T>::value ||
...
void do_serialize(const dataT &object, serializerT &serializer,
  ...
  if constexpr (is_optional<dataT>::value) {
    ...
    serializer.String(s.c_str(), static_cast<rapidjson::SizeType>(s.size()),
                      true);
  }

  else {
    //        static_assert (false, "do_serialize: Unexpected type. Don't know
    //        how to convert it");
...
template <typename dataT>
void SerializeFromJson(dataT &rootData, const std::string &data,
                       const serialize_properties_t &properties = {}) {
